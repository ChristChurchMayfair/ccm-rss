...
function OsisFormatter() {
    ...
    function formatVariable(optionsKey, part, token) {
        ...
        pattern = pattern.replace(/\$chapters/g, function() {
            ...
            if (
                arrayToUse.length > 1 &&
                multipleChaptersPosition(part, token) > 0
            ) {
        ...
        pattern = pattern.replace(
            /\$c/g,
            typeof part.c === "string" ? part.c : "",
        )
        pattern = pattern.replace(
            /\$v/g,
            typeof part.v === "string" ? part.v : "",
        )
        return pattern
    }
    ...
    function addBookSequence(token, sequenceArray, tokens) {
        ...
        while (sequenceArray.length > 1) {
            ...
            if (
                ...
                token.subTokens = tokens.splice(
                    0,
                    (sequenceArray.length - 1) * 2,
                )
    ...
    function osisWithContext(osis, context) {
        ...
        if (
            osisChapterWithContext(b, c, v, omitChapter, context, token) ===
            false
        ) {
            return token
        }
